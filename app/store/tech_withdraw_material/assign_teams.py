...
def assign_teams(equipments, tech, user_id):
    ...
    serials = request_equipment_withdraw(session.get('technical_equipments'))
    for serial in serials:
        if serial['bool'] is True:
            query = 'UPDATE material_assignment SET register_active = False WHERE serial_id = %s;'
        else:
            pass
